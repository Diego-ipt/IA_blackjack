...
def test_agente_markov_vs_aleatorios():
    ...
    agentes = [agente_markov_normal, agente_markov_arriesgado, agente_markov_umbral, agente_markov_hibrido, agente_hilo, agente_aleatorio1, agente_aleatorio2]
    ...
    fieldnames = [
        ...
        'hibrido_decision_time_ms', 'hibrido_decisions', 'hibrido_result', 'hibrido_capital_change',
        'hilo_decision_time_ms', 'hilo_decisions', 'hilo_result', 'hilo_capital_change']
    csv_data = []
    
    print("NOTA: Agentes Markov apuestan $5 por ronda, HiLo usa apuesta adaptativa")
    
    # Contadores para todos los agentes
    ...
    hibrido_wins = hibrido_losses = hibrido_ties = 0
    hilo_wins = hilo_losses = hilo_ties = 0

    # Timing para todos los agentes
    normal_decision_times = []
    arriesgado_decision_times = []
    umbral_decision_times = []
    hibrido_decision_times = []
    hilo_decision_times = []
    ...
    original_hibrido_decidir = agente_markov_hibrido.decidir_accion
    original_hilo_decidir = agente_hilo.decidir_accion
    ...
    def timed_hibrido_decidir(mano, carta_dealer):
        start_time = time.time()
        result = original_hibrido_decidir(mano, carta_dealer)
        decision_time = time.time() - start_time
        hibrido_decision_times.append(decision_time)
        return result

    def timed_hilo_decidir(mano, carta_dealer):
        start_time = time.time()
        result = original_hilo_decidir(mano, carta_dealer)
        decision_time = time.time() - start_time
        hilo_decision_times.append(decision_time)
        return result
    ...
    agente_markov_hibrido.decidir_accion = timed_hibrido_decidir
    agente_hilo.decidir_accion = timed_hilo_decidir
    ...
    def tracked_jugar_ronda():
        ...
        nonlocal hibrido_wins, hibrido_losses, hibrido_ties
        nonlocal hilo_wins, hilo_losses, hilo_ties
        ...
        hibrido_start_capital = agente_markov_hibrido.jugador.capital
        hilo_start_capital = agente_hilo.jugador.capital
        ...
        hibrido_decision_times.clear()
        hilo_decision_times.clear()
        ...
        hibrido_final_capital = agente_markov_hibrido.jugador.capital
        hilo_final_capital = agente_hilo.jugador.capital
        ...
        hibrido_capital_change = hibrido_final_capital - hibrido_start_capital
        hilo_capital_change = hilo_final_capital - hilo_start_capital
        ...
        # Calcular resultados para agente híbrido
        if hibrido_capital_change > 0:
            hibrido_result = 1
            hibrido_wins += 1
        elif hibrido_capital_change < 0:
            hibrido_result = -1
            hibrido_losses += 1
        else:
            hibrido_result = 0
        ...
        # Calcular resultados para agente HiLo
        if hilo_capital_change > 0:
            hilo_result = 1
            hilo_wins += 1
        elif hilo_capital_change < 0:
            hilo_result = -1
            hilo_losses += 1
        else:
            hilo_result = 0
        ...
        round_data = {
            ...
            'hibrido_decision_time_ms': int(sum(hibrido_decision_times) * 1000) if hibrido_decision_times else 0,
            'hibrido_decisions': len(hibrido_decision_times),
            'hibrido_result': hibrido_result,
            'hibrido_capital_change': hibrido_capital_change,
            'hilo_decision_time_ms': int(sum(hilo_decision_times) * 1000) if hilo_decision_times else 0,
            'hilo_decisions': len(hilo_decision_times),
            'hilo_result': hilo_result,
            'hilo_capital_change': hilo_capital_change
        }
        ...
        if round_number % 100 == 0:
            ...
            print(f"  Híbrido: Capital={hibrido_final_capital} (Δ{hibrido_capital_change:+d})")
            print(f"  HiLo: Capital={hilo_final_capital} (Δ{hilo_capital_change:+d})")
    ...
    if total_rounds > 0:
        ...
        print(f"\nAgente Markov Híbrido:")
        print(f"  Victorias: {hibrido_wins}/{total_rounds} ({hibrido_wins/total_rounds*100:.2f}%)")
        print(f"  Derrotas: {hibrido_losses}/{total_rounds} ({hibrido_losses/total_rounds*100:.2f}%)")
        print(f"  Empates: {hibrido_ties}/{total_rounds} ({hibrido_ties/total_rounds*100:.2f}%)")

        print(f"\nAgente HiLo:")
        print(f"  Victorias: {hilo_wins}/{total_rounds} ({hilo_wins/total_rounds*100:.2f}%)")
        print(f"  Derrotas: {hilo_losses}/{total_rounds} ({hilo_losses/total_rounds*100:.2f}%)")
        print(f"  Empates: {hilo_ties}/{total_rounds} ({hilo_ties/total_rounds*100:.2f}%)")
